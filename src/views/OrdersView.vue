  ...
  const state = reactive({
    ...
    statusFilter: 'all', // all, accepted, in-progress, ready-for-delivery
  });
  ...
  const statusCounts = computed(() => {
    ...
    state.acceptedOrders.forEach(order => {
      const normalizedStatus = normalizeStatusForFilter(order.status);
      ...
      <div class="container mx-auto px-4 py-8 max-w-6xl">
        ...
        <div class="mb-8">
          ...
          <div class="mt-6 flex flex-wrap gap-2">
            ...
            <button v-for="status in orderStatuses" :key="status.value" @click="state.statusFilter = status.filterValue"
              :class="[
